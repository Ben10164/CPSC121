...
bool determineGame(char visibleLetters[100], char wordchar[100], int length) {
	for (int i = 0; i < length; i++) {
		if (wordchar[i] != visibleLetters[i]) { // Sees if all of the visible letters are the same as the regular word
...
void checkAvailableLetters(char(&availableLetters)[26], int guess) {
	for (int i = 0; i < 26; i++) { // Checks to see if the guessed character is available in the words list
		if (guess == availableLetters[i]) { // The guessed char is in the list of unguessed char
...
void checkHiddenLetters(char(&wordchar)[100], char(&visibleLetters)[100], int guess, bool& didChange) {
	for (int i = 0; i < sizeof(wordchar); i++) {
		if (guess == wordchar[i]) { // Sees if the guessed char is in the word to guess
...
void openFile(ifstream& inputFile) {
	...
	if (inputFile.is_open()) { // checks if the file is open
...
void readFile(ifstream& inputFile, string word, string(&words)[100], int& index) {
	while (!inputFile.eof()) { // Starts the loop to read in the files
		word = "";
...
void createAvailableLetters(char(&availableLetters)[26], char& letter) {
	letter = 'a';
	for (int i = 0; i < 26; i++) { // This is making the available letters array the alphabet
...
void createVisibleLetters(char(&visibleLetters)[100], int& length) {
	for (int i = 0; i < length; i++) { // Makes the array for the hidden word be a - per character in the word
		visibleLetters[i] = '-';
	}
}
...
void mainLoop(int& guesses, bool& goodGame, char(&visibleLetters)[100], char(&wordchar)[100], int& length, char(&availableLetters)[26], char& guess, bool& didChange) {
	guesses = 7;
	while (true) { // Starts the while loop, this is the while loop that continues until the game ends
		if (guesses == 0) { // If the character runs out of guesses, break
			break;
		}
		goodGame = determineGame(visibleLetters, wordchar, length); // Does the determineGame function
		if (goodGame) { // goodGame represents if the game has been won
			break; // If the game was won, then break
		}



		for (int i = 0; i < length; i++) { // Outputs the hidden words array
		...
		for (int i = 0; i < 26; i++) { // Starts the loop to output the available letters
		...
		if (didChange) { // Dependong on if something changed, it will either output a correct or incorrect
